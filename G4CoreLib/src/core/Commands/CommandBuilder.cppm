...
export module GeantCore.Core.Commands.G4CommandBuilder;
...
public:
  explicit G4CommandBuilder(G4UImessenger *owner) : owner(owner) {}
  ...
  // Число + единицы
  G4UIcmdWithADoubleAndUnit *DoubleUnit(std::string_view path,
  ...
  // Вектор + единицы
  G4UIcmdWith3VectorAndUnit *Vec3Unit(std::string_view path,
                                      std::string_view unitCategory) {
  ...
  G4UIcmdWithAString *Candidates(std::string_view path,
